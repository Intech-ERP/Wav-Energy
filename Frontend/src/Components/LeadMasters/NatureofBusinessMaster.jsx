...
export default function NatureOfBusinessMaster({ tabValue, onEditData, refreshKey }) {

    const tab = tabValue === 'nature_of_business' ? 'natureofBusiness' : tabValue

    const { data, refetch, setData } = useFetchData(`${tab}`);
      ...
      const updateDisplayOrder = async (dispValue, row) => {
          try {
              ...
              const updateId = row.original.nature_of_business_id;
              ...
              if (response.success) {
                  setData((prev) =>
                      prev
                          .map((item) =>
                              item.nature_of_business_id === updateId
      ...
      const handleDeleteData = async () => {
          try {
              const response = await removeMasterData(
                  deleteRow?.nature_of_business_id,
                  tabValue
              );
              if (response?.success) {
      ...
      const column = [
          { id: 1, accessorKey: "nature_of_business", header: "Nature of Business" },
          {
              id: 2,
